module.exports = (io, socket, room) => {
  ...
  socket.on('flip_is_ready', (id) => {    // 뒤집기쥰비완료쓰
  ...
  socket.on("card_will_check", (data) => {  // 카드 체크한대
    const { id, c_index, check } = data;
    let Index = room.findIndex((e) => e.userid.includes(id));
    console.log(c_index);
    if (check)
      socket.broadcast.to(room[Index].roomCode).emit("card_matched", c_index);  //맞췄대
    else socket.broadcast.to(room[Index].roomCode).emit("card_not_matched", c_index);  // 못맞췄대
  });
}